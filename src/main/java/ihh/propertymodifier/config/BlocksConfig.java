...
public record BlocksConfig(
    ...
    public void process() {
        process("destroy_time",                    destroyTime(),                 ConfigResults.DESTROY_TIME::put);
        process("explosion_resistance",            explosionResistance(),         ConfigResults.EXPLOSION_RESISTANCE::put);
        process("requires_correct_tool_for_drops", requiresCorrectToolForDrops(), ConfigResults.REQUIRES_CORRECT_TOOL_FOR_DROPS::put);
        process("is_randomly_ticking",             isRandomlyTicking(),           ConfigResults.IS_RANDOMLY_TICKING::put);
        process("ignited_by_lava",                 ignitedByLava(),               ConfigResults.IGNITED_BY_LAVA::put);
        process("has_collision",                   hasCollision(),                ConfigResults.HAS_COLLISION::put);
        process("friction",                        friction(),                    ConfigResults.FRICTION::put);
        process("speed_factor",                    speedFactor(),                 ConfigResults.SPEED_FACTOR::put);
        process("jump_factor",                     jumpFactor(),                  ConfigResults.JUMP_FACTOR::put);
        ConfigHelper.processConfigMap(lightLevel(), BuiltInRegistries.BLOCK, (block, value) -> block.getStateDefinition().getPossibleStates().forEach(e -> ConfigResults.LIGHT_LEVEL.put(e, value)), null);
        ConfigHelper.processConfigMap(lightLevel(), ConfigHelper::resolveBlockStatesRegex, ConfigResults.LIGHT_LEVEL::put, key -> "Key " + key + " for light_level in blocks.json did not match any block states");
    }

    private static <T> void process(String configName, Map<String, T> configValue, BiConsumer<Block, T> consumer) {
